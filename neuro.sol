...
//Neuro is a BEP-2O token that aims to raise funding for pediatric brain tumor research and clinical trials.
//Most code borrowed from another great charity token, Charitas
/*
...
contract Neuro is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    string private _name = "Neuro.charity";
    string private _symbol = "NEURO";
    uint8 private _decimals = 18;
    mapping(address => uint256) internal _reflectionBalance;
    mapping(address => uint256) internal _tokenBalance;
    mapping(address => mapping(address => uint256)) internal _allowances;
    uint256 private constant MAX = ~uint256(0);
    uint256 internal _tokenTotal = 10000 *10**18; // 10 thousands totalSupply
    ...
    address public charityAddress = 0x80F4aea6A4390f5265A690B68E6E3f822F7879b2; // Charity Address
    ...
    function excludeAccount(address account) external onlyOwner() {
        require(account != 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D,"NEURO: Uniswap router cannot be excluded.");
        require(account != address(this), 'NEURO: The contract it self cannot be excluded');
        require(!_isExcluded[account], "NEURO: Account is already excluded");
    ...
    function includeAccount(address account) external onlyOwner() {
        require(_isExcluded[account], "NEURO: Account is already included");
    ...
    */
    ...
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s) external {
        ...
        require(signatory != address(0), "NEURO::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "NEURO::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "NEURO::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber) external view returns (uint256) {
        require(blockNumber < block.number, "NEURO::getPriorVotes: not yet determined");
        uint32 nCheckpoints = numCheckpoints[account];
        if (nCheckpoints == 0) {
            return 0;
        }
    ...
    function _writeCheckpoint(address delegatee, uint32 nCheckpoints, uint256 oldVotes, uint256 newVotes) internal {
        uint32 blockNumber = safe32(block.number, "NEURO::_writeCheckpoint: block number exceeds 32 bits");
